...
def toggle_music(state, args):
    ...
    # red = os.read(fd, 4)
    # # read 4 bytes and store in red var
    ...
    #     play = False
    # # print(len(red))
    # # print("red[music_number] = ", red[music_number])
...
while True:
    ...
    for idx, value in enumerate(red):
        if value == "1":
            status = switches[idx].get_value()
            if not status:
                switches[idx].set_value(True)
                toggle_music(True, (idx,))
        else:
            status = switches[idx].get_value()
            if status:
                switches[idx].set_value(False)
                toggle_music(False, (idx,))
